...
public class GG_AIPath : MonoBehaviour
{
	...
	{
		// Temp.
		//if (canSearch && target == null)
		//target = UnitSelection.Instance.GetTargetPosition ();
	...
	//Custom Function for "Stopping the path".
	public void ResetPath ()
	{
		//seeker.StartPath (GetFeetPosition () + transform.forward, GetFeetPosition () + transform.forward);
		seeker.StartPath (transform.position, GetFeetPosition () + transform.forward * 0.1f);
		//seeker.StartPath (transform.position + transform.forward, transform.position + transform.forward);
	}
