...
public class AstarPath : MonoBehaviour
{
	...
	private void OnDrawGizmos ()
	{
		...
		if (pathQueue != null && pathQueue.AllReceiversBlocked && workItems.Count > 0)
			return;
	...
	public void BlockUntilPathQueueBlocked ()
	{
		...
		if (!Application.isPlaying) {
			if (!pathQueue.AllReceiversBlocked)
				throw new System.Exception ("Pathfinding running while not playing?");
			return;
		}
#endif
	...
	public void ScanLoop (OnScanStatus statusCallback)
	{
		...
		for (int i = 0; i < graphs.Length; i++) {
			...
			if (graph == null) {
				if (statusCallback != null)
					statusCallback (new Progress (Mathf.Lerp (0.05F, 0.7F, (float)(i + 0.5F) / (graphs.Length + 1)), "Skipping graph " + (i + 1) + " of " + graphs.Length + " because it is null"));
				continue;
			}

			if (OnGraphPreScan != null) {
				if (statusCallback != null)
					statusCallback (new Progress (Mathf.Lerp (0.1F, 0.7F, (float)(i) / (graphs.Length)), "Scanning graph " + (i + 1) + " of " + graphs.Length + " - Pre processing"));
			...
			if (OnGraphPostScan != null) {
				if (statusCallback != null)
					statusCallback (new Progress (Mathf.Lerp (0.1F, 0.7F, (float)(i + 0.95F) / (graphs.Length)), "Scanning graph " + (i + 1) + " of " + graphs.Length + " - Post processing"));
